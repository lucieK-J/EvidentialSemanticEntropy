...
from evsme.other_uq import mass_on_total_set
from evsme.other_uq import semantic_entropy_correction
from evsme.evidential_framework_construction import all_entailements
from evsme.evidential_framework_construction import build_hierarchy
from evsme.evidential_framework_construction import build_hierarchy_ablation1
from evsme.evidential_framework_construction import build_hierarchy_ablation2
from evsme.evidential_framework_construction import build_frame_of_discernement
...
def main(args):
    ...
    result_dict["graphs"] = []
    if args.compute_ablations: ## ablation for evsme
        result_dict['our_clusters'] = []
        result_dict['their_clusters'] = []
        result_dict['entailements_full_matrix'] = []
    ...
    for idx, tid in enumerate(validation_generations):
        ...
        if args.compute_predictive_entropy:
            ...
            if not args.use_all_generations:
                ...
                log_liks_with_eos = [r[2] for r in full_responses[:args.use_num_generations]] # necessary for evsme
            else:
                log_liks = [r[1] for r in full_responses]
                log_liks_with_eos = [r[2] for r in full_responses] # necessary for evsme
            
            for i in log_liks:
                assert i

            for i in log_liks_with_eos: # necessary for evsme
                assert i
            ...
            mi = mass_on_total_set(log_liks_with_eos, responses)
            entropies['mass_ignorance'].append(mi)

            # same as pe but events counted only once
            pe_corrected = semantic_entropy_correction(log_liks_with_eos, semantic_ids, responses, sequence_prob_type = "raw", do_correct=True, sumto1 = True, entropie_normalize = "raw")
            entropies['pe_corrected'].append(pe_corrected)

            unique_responses = list(dict.fromkeys(responses))
            entailements_full_matrix = all_entailements(unique_responses, model=entailment_model, example=example) # count one sequence only one time
            hierarchy_info = build_hierarchy(entailements_full_matrix, unique_responses)
            discernment_info = build_frame_of_discernement(hierarchy_info)
            mass = mass_assignment(log_liks_with_eos, responses, hierarchy_info, discernment_info)
            ...
            if args.compute_ablations:
                ...
                hierarchy_info_abla1 = build_hierarchy_ablation1(entailements_full_matrix, unique_responses)
                discernment_info_abla1 = build_frame_of_discernement(hierarchy_info_abla1)
                mass_abla1 = mass_assignment(log_liks_with_eos, responses, hierarchy_info_abla1, discernment_info_abla1)

                jousselme_abla1 = jousselme_entropy(F = mass_abla1['F'], M = mass_abla1['M'], frame_size = discernment_info_abla1['Omega_size'])
                entropies['jousselme ablation 1'].append(jousselme_abla1)

                hierarchy_info_abla2 = build_hierarchy_ablation2(unique_responses, entailment_model, args, example)
                discernment_info_abla2 = build_frame_of_discernement(hierarchy_info_abla2)
                mass_abla2 = mass_assignment(log_liks_with_eos, responses, hierarchy_info_abla2, discernment_info_abla2)

                jousselme_abla2 = jousselme_entropy(F = mass_abla2['F'], M = mass_abla2['M'], frame_size = discernment_info_abla2['Omega_size'])
                entropies['jousselme ablation 2'].append(jousselme_abla2)
                ...
                result_dict['entailements_full_matrix'].append(entailements_full_matrix)
                result_dict['their_clusters'].append(their_clusters)
