...
def main(args):
    ...
    if not os.path.exists(f"{parent_dir}/EXP"):
        os.makedirs(f"{parent_dir}/EXP")


    # store configurations locally (evsme project)
    experiment_data_path = f"{parent_dir}/EXP/experiment_data.txt"
    ...
    for dataset_split in ['train', 'validation']:
        ...
        for index in tqdm(indices):
            ...
            for i in range(num_generations):
                ...
                predicted_answer, token_log_likelihoods, token_log_likelihoods_with_EOS, embedding = model.predict(
                    local_prompt, temperature) # log_likelihoods_including_EOS necessary of evsme
                ...
                if i == 0:
                    ...
                    most_likely_answer_dict = {
                        ...
                        'token_log_likelihoods_with_EOS': token_log_likelihoods_with_EOS, # modif from inclusion of evsme
                        'embedding': embedding,
                        'accuracy': acc}
                    ...
                    full_responses.append(
                        (predicted_answer, token_log_likelihoods, token_log_likelihoods_with_EOS, embedding, acc)) # modif from inclusion of evsme
