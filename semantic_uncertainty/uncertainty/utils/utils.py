...
def get_parser(stages=['generate', 'compute']):
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            "--compute_kle", default=False,
            action=argparse.BooleanOptionalAction,
            help='Skip kle to save time')
        parser.add_argument(
            "--compute_ablations", default=False,
            action=argparse.BooleanOptionalAction,
            help='Ablations of evsme')
...
def setup_logger():
    ...
    logging.basicConfig(
        ...
        # level=logging.INFO,
        level=logging.WARNING,  # Set to WARNING to reduce verbosity (evsme project)
        datefmt='%Y-%m-%d %H:%M:%S')
    # logging.getLogger().setLevel(logging.INFO)  # logging.DEBUG
    logging.getLogger().setLevel(logging.WARNING) # Set to WARNING to reduce verbosity (evsme project)
...
def model_based_metric(predicted_answer, example, model):
    ...
    if 'gpt' in model.model_name.lower():
        ...
        predicted_answer, _, _, _ = model.predict(prompt, 0.01) ## evsme : looks like it is not working for p_true_utils.construct_few_shot_prompt
    ...
    if 'yes' in predicted_answer.lower():
        ...
        predicted_answer, _, _, _ = model.predict(prompt, 1)
        if 'yes' in predicted_answer.lower():
            return 1.0
        elif 'no' in predicted_answer.lower():
            return 0.0
